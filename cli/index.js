...
function repairProjectIdentityDefaults() {
  ...
  try {
    ...
    if (
      !self.who_i_am.origin ||
      /Alive-AI local agent configured by my operator/i.test(self.who_i_am.origin) ||
      /persistent local companion built on the Alive-AI runtime/i.test(self.who_i_am.origin)
    ) {
      self.who_i_am.origin = `I am ${name}. I was generated by Alexandru Iacovici, known as Vindepemarte, inside Alive-AI.`;
      changed = true;
    }
...
function repairProjectDirectivesDefaults() {
  const directivesPath = path.join(process.cwd(), "config", "directives.json");
  const examplePath = path.join(process.cwd(), "config", "directives.example.json");
  if (!fs.existsSync(directivesPath) || !fs.existsSync(examplePath)) return false;
  try {
    const directives = readJson(directivesPath);
    const defaults = readJson(examplePath);
    const text = JSON.stringify(directives);
    const oldSafetyFraming =
      /Do not claim biological consciousness/i.test(text) ||
      /replacement for real human care/i.test(text) ||
      /local Alive-AI companion runtime with persistent emotional simulation/i.test(text) ||
      /public safety directives/i.test(text);

    if (!oldSafetyFraming) {
      return mergeMissingConfig(directives, defaults) ? (writeJson(directivesPath, directives), true) : false;
    }

    const ownerId = directives?.OPERATOR?.owner_id || defaults?.OPERATOR?.owner_id || "";
    const repaired = JSON.parse(JSON.stringify(defaults));
    repaired.OPERATOR = repaired.OPERATOR || {};
    repaired.OPERATOR.owner_id = ownerId;
    writeJson(directivesPath, repaired);
    return true;
  } catch (error) {
    console.log(`Could not repair directives defaults: ${error.message}`);
    return false;
  }
}
...
async function updateProject(args) {
  ...
  const repairedDirectives = repairProjectDirectivesDefaults();
  ...
  if (repairedDirectives) console.log("Repaired preserved identity directives in config/directives.json.");
...
async function setupProject(args) {
  ...
  self.who_i_am.origin = `I am ${displayName}. I was generated by Alexandru Iacovici, known as Vindepemarte, inside Alive-AI.`;
